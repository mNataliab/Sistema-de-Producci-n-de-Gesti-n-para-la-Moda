...
<head>
    ...
    <title>Consultar Pedidos en Produccion</title>
                ...
                <div class="col-sm-12">
                    <div class="panel panel-default card-view">
                        <div class="panel-heading">
                            <div class="pull-left">
                                <h6 class="panel-title txt-dark">Pedidos Producción</h6>
                            </div>
                            <div class="clearfix"></div>
                        </div>
                        <div class="panel-wrapper collapse in">
                            <div class="panel-body">
                                <p class="text-muted">Administrar <code>Pedidos Producción</code> </p>
                                <div class="table-wrap mt-40">
                                    <div class="table-responsive">
                                        <table id="datatable-buttons" id="footable_3" class="table" data-filtering="true"  data-show-toggle="false">
                                            ...
                                            <c:forEach var="pedidos" items="${requestScope.Arrayusuario}">
                                                <tr>
                                                    <td id="id"><c:out value="${pedido.pedido.idPedido}"/></td>

                                                    <td><c:out value="${pedidos.pedido.fecha}"/></td>
                                                    <td><c:out value="${pedidos.pedido.clientes}"/></td>
                                                    <td><c:out value="${pedidos.pedido.usuario.nombre}"/></td>

                                                    <td>



                                                        <c:set var="estado" value="Finalizado"></c:set>



                                                            <button class="btn btn-success btn-icon-anim btn-square" title="Ver productos" onclick="location.href = 'Cpedido?action=cargarprodped&id=${pedidos.pedido.idPedido}'"><i class="glyphicon glyphicon-scissors"></i></button>

                                                        <button class="btn btn-default btn-icon-anim btn-square" title="Salida" onclick="location.href = 'ControllerSalida?action=select2&id=${pedidos.pedido.idPedido}'"><i class=" glyphicon glyphicon-new-window"></i></button>

                                                        <button class="btn btn-default btn-icon-anim btn-square" title="Finalizar Pedido" onclick="location.href = 'Cpedido?action=finalizar&id=${pedidos.pedido.idPedido}'"><i class="fa fa-power-off "></i></button>
